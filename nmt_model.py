...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        @returns enc_hidden (Tensor): Tensor of hidden units with shape (b, src_len, h*2), where
                                        b = batch size, src_len = maximum source sentence length, h = hidden size.
        @returns dec_init_state (tuple(Tensor, Tensor)): Tuple of tensors representing the decoder's initial
                                                hidden state and cell.
        """
        enc_hidden, dec_init_state = None, None
        ...
        ###     2. Compute `enc_hidden`, `last_hidden`, `last_cell` by applying the encoder to `X`.
        ###         - Before you can apply the encoder, you need to apply the `pack_padded_sequence` function to X.
        ###         - After you apply the encoder, you need to apply the `pad_packed_sequence` function to enc_hidden.
        ###         - Note that the shape of the tensor returned by the encoder is (src_len, b, h*2) and we want to
        ###           return a tensor of shape (b, src_len, h*2) as `enc_hidden`.
        ...
        src_lens = torch.tensor(source_lengths, dtype=torch.long)
        enc_input = pack_padded_sequence(X, src_lens)
        enc_output, (last_hidden, last_cell) = self.encoder(enc_input)  # enc_output: (src_len, b, 2 * hid_size)
        enc_output, _ = pad_packed_sequence(enc_output)
        enc_hidden = enc_output.permute(1, 0, 2).contiguous()  # enc_hidden: (b, src_len, 2 * hid_size)
        last_hidden = last_hidden.permute(1, 0, 2).contiguous().view(-1, self.hidden_size * 2)  # h: (b, 2 * hid_size)
        last_cell = last_cell.permute(1, 0, 2).contiguous().view(-1, self.hidden_size * 2)      # c: (b, 2 * hid_size)
        init_decoder_hidden = self.h_projection(last_hidden)  # init_decoder_hidden: (b, hid_size)
        init_decoder_cell = self.c_projection(last_cell)      # init_decoder_cell: (b, hid_size)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # enc_hiddens_proj: (b, src_len, hid_size)
        Y = self.model_embeddings.target(target_padded)  # T: (tgt_len, b, e)
        for Y_t in torch.split(Y, 1):
            Y_t = Y_t.squeeze(0)   # Y_t: (b, e)
            ...
            combined_outputs.append(combined_output)   # o_t: (b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, _ = dec_state  # dec_hidden: (b, h)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1))  # (b, src_len, h) * (b, h, 1) => (b, src_len, 1)
        e_t = e_t.squeeze(-1)  # e_t: (b, src_len)
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)   # a_t: (b, 2h)
